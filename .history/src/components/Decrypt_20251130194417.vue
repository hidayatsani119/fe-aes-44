  ...
  <div class="hero grow flex flex-col justify-center items-center">
    <div>
      <form @submit.prevent="handleSubmit">
        <fieldset
          class="fieldset flex flex-col justify-center bg-base-100 border-base-300 rounded-box w-xl h-full border p-4"
        >
          <h1 class="text-3xl font-bold mb-5 text-center text-neutral">Decrypt</h1>
          <label class="label">Ciphertext Hex</label>
          <input type="text" class="input w-full" placeholder="Plaintext" v-model="input.ciphertext_hex" />

          <label class="label">Secret</label>
          <input type="text" class="input w-full" placeholder="Secret Key" v-model="input.key" />
